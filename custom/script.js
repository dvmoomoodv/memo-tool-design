...
app.controller('ctrl', ["$scope", function($scope) {
  $scope.memos = [
                  {title:'후보키', content:'릴레이션을 구성하는 속성들 중에서 튜플을 유일하게 식별하기 위해 사용되는 속성들의 부분집합이다.후보키는 유일성과 최소성을 모두 만족한다.', tag:'후보키,릴레이션,튜플,유일성,희소성'},
                  ...
                  {title:'슈퍼키', content:'복합키 또는 연결키라고 한다.기본키로 사용되며 2개 이상의 열을 1개의 후보키로 만들어 기본키로 활용되는 키슈퍼키는 한 릴레이션 내에 있는 속성들의 집합으로 구성된 키를 말한다.릴레이션을 구성하는 모든 튜플 중 슈퍼키로 구성된 속성의 집합과 동일한 값은 나타나지 않는다.슈퍼키는 릴레이션을 구성하는 모든 튜플에 대해 유일성은 만족하지만, 최소성은 만족하지 못한다.', tag:'복합키,연결키,최소성,'},
